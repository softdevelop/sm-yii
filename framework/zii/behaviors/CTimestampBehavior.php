...
class CTimestampBehavior extends CActiveRecordBehavior {
	...
	protected function getTimestampByColumnType($columnType) {
		return isset(self::$map[$columnType]) ? new CDbExpression(self::$map[$columnType]) : date("Y-m-d H:i:s");
		$dbKey=isset(self::$map[Yii::app()->db->driverName]) ? Yii::app()->db->driverName : 'default';
		return isset(self::$map[$dbKey][$columnType]) ? new CDbExpression(self::$map[$dbKey][$columnType]) : date("Y-m-d H:i:s");
	}
}
